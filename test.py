...
def shard_expert_params(model: DNA, mesh: Mesh) -> DNA:
    eparts = mesh.shape["expert"]
    stacked_param_leaves = []
    for g in model.groups:
        stacked_param_leaves += jax.tree.leaves(g.params)
    stacked_ids = {id(x) for x in stacked_param_leaves}
...
def build_model_and_routers(
    *,
    ...
    mods += [
        FeedForward(d_model, 4, dropout, key=klist[n_attn + i]) for i in range(n_ff)
    ]

    total_experts = n_attn + n_ff
    router_keys = jax.random.split(k_routers, n_hops)
    routers = tuple(
        Router(d_model, total_experts, topk, dropout, key=k) for k in router_keys
    )
...
def main():
    ...
    assert (
        args.batch_size % args.batch_shards == 0
    ), "batch_size must be divisible by batch_shards"
    assert (
        args.n_attn % args.expert_shards == 0
    ), "n_attn must be divisible by expert_shards"
    assert (
        args.n_ff % args.expert_shards == 0
    ), "n_ff must be divisible by expert_shards"
    ...
    experts_per_shard = sum(
        int(g.idx.shape[0]) // args.expert_shards for g in model.groups
    )
    ...
    with mesh:
        ...
        _ = pjitted_fwd(
            params, in_ids, in_msk, jax.random.PRNGKey(124)
        ).block_until_ready()
        ...
        tgt = jax.random.randint(
            key0, (args.batch_size, args.seq_len), 0, args.vocab, dtype=jnp.int32
        )
        ...
        jax.tree.map(
            lambda x: x.block_until_ready() if isinstance(x, jax.Array) else None, grads
        )
