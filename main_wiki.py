...
def load_wikitext(tok, seq_len: int, split: str = "train"):
...
def main():
    wandb.init(project="dna-wikitext", name=cfg.model_type, config=asdict(cfg))

    tok = AutoTokenizer.from_pretrained("gpt2")
    tok.pad_token = tok.eos_token

    train_it = iter(load_wikitext(tok, cfg.seq_len, "train"))
    val_it = iter(load_wikitext(tok, cfg.seq_len, "validation"))
